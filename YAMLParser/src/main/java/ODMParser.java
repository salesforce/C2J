...
public class ODMParser {
    ...
    private void GenerateClass(List<String> currentYaml) {
        var className = Escape(currentYaml.get(0));
        //  base: sObject
        var baseClass = currentYaml.stream().filter(s->s.split(" ")[0].equals("base")).findFirst();
        var baseClassName="";
        if(baseClass.isPresent())
            baseClassName=baseClass.get().split(" ")[1];

        var yamlClass = currentYaml.stream().filter(s->!s.startsWith("base ")).collect(Collectors.toList());
        this.classes.put(className, GenerateClass(className, baseClassName, yamlClass.subList(1, yamlClass.size())));
    }
    ...
    private StringBuilder Header(String name, String baseClass, List<String> classYaml) {
        ...
        javaClass.append(String.format("\n\n/***  THIS CLASS WAS AUTO GENERATED by the DSMParser app   %s ***/", java.time.LocalDate.now()));
        if (baseClass.equals("Object"))
            javaClass.append("\n@JsonIgnoreProperties(ignoreUnknown = true)");

        javaClass.append(String.format("\npublic class %s", name));
        if (!baseClass.isEmpty() && !baseClass.equals("Object"))
